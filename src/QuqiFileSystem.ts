...
import Errors, {ResourceType, ReturnCallback, SimpleCallback} from "webdav-server";
...
export class QuqiFileSystemResource
{
  ...
  constructor(data ?: QuqiFileSystemResource)
  {
    if(!data)
    {
    ...
    {
      const rs = data as QuqiFileSystemResource;
...
export class QuqiSerializer implements FileSystemSerializer
{
  ...
  unserialize(serializedData : any, callback : ReturnCallback<FileSystem>) : void
  {
    ...
    const fs = new QuqiFileSystem(serializedData.username, serializedData.password, serializedData.rootPath);
...
export class QuqiFileSystem extends FileSystem{
  ...
  constructor(public username : string, public password : string, public rootPath : string)
  {
    super(new QuqiSerializer());

    this.resources = {
      '/': new QuqiFileSystemResource()
    };
  }
  ...
  protected _create(path : Path, ctx : CreateInfo, _callback : SimpleCallback) : void
  {
    ...
    const callback = (e) => {
      if(!e)
        this.resources[path.toString()] = new QuqiFileSystemResource();
      else if(e)
  ...
  protected getPropertyFromResource(path : Path, ctx : any, propertyName : string, callback : ReturnCallback<any>) : void
  {
    ...
    if(!resource)
    {
      resource = new QuqiFileSystemResource();
